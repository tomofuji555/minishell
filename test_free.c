...
/*   Updated: 2024/04/13 19:02:06 by toshi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	free_init_data_and_tnode(t_tree_node *ptr)
{
	if (ptr->init_data.cmd_tokens)
		free_tkn_list(ptr->init_data.cmd_tokens);
	if (ptr->init_data.infile_tokens)
		free_tkn_list(ptr->init_data.infile_tokens);
	if (ptr->init_data.infile_tokens)
		free_tkn_list(ptr->init_data.outfile_tokens);
	free(ptr);
}
