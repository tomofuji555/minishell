...
/*   Updated: 2024/03/27 07:03:18 by tozeki           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char *make_first_path(t_path *first, char *current_dir)
{
	if (is_equal_str(first->val, "./"))
		...
		return (ft_xsubstr(current_dir, 0, (size_t)(ft_strrchr(current_dir, '/') - current_dir + 1))); //current_dirの最後から２個目のスラッシュまでを抜き出し返す
	else
